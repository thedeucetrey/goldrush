// Pay Dirt — main.js (v3.1 hotfix: interactive hi-res map when tabbed)
...
// (optional) POIs; safe if you didn’t add the checkbox
const POIS = [
...
// -------- Leaflet IIIF (fix: init after visible, then invalidateSize) --------
let leafletCtx = { map:null, iiif:null, playerMarker:null, poiLayer:null, claimsLayer:null, ready:false };

async function initLeafletVisible() {
  if (leafletCtx.ready) { // already initialized before
    // ensure proper sizing every time we show the tab
    setTimeout(()=>{
      leafletCtx.map.invalidateSize();
      // optional: keep view sane
      if (leafletCtx.iiif?._bounds) leafletCtx.map.fitBounds(leafletCtx.iiif._bounds, { animate:false });
      updateLeafletPlayer(); syncPOIsToLeaflet(); syncClaimsToLeaflet();
    }, 0);
    return;
  }
  // Create the map NOW that #leafletMap is visible
  const map = L.map("leafletMap", { zoomSnap: 0.25 });
  const manifest = await fetch(HIST_MANIFEST_URL).then(r=>r.json()).catch(()=>null);
  const canvas = manifest?.sequences?.[0]?.canvases?.[0];
  const serviceId = canvas?.images?.[0]?.resource?.service?.['@id'];
  if (!serviceId){ status("Could not load historical map."); return; }
  const infoUrl = serviceId.replace(/\/$/, '') + "/info.json";
  const layer = L.tileLayer.iiif(infoUrl, { fitBounds: true }).addTo(map);

  leafletCtx = { map, iiif:layer, playerMarker:null, poiLayer:null, claimsLayer:null, ready:true };

  // IMPORTANT: give Leaflet a tick to size tiles properly
  setTimeout(()=>{
    map.invalidateSize();
    if (layer?._bounds) map.fitBounds(layer._bounds, { animate:false });
  }, 0);

  // Click → grid coords (proportional to IIIF bounds)
  map.on("click", (e)=>{
...
function updateLeafletPlayer(){
  ...
  if (!leafletCtx.playerMarker){
    leafletCtx.playerMarker = L.circleMarker([lat,lng], {radius:5, weight:2}).addTo(leafletCtx.map);
  } else {
    leafletCtx.playerMarker.setLatLng([lat,lng]);
  }
}
...
function syncPOIsToLeaflet(){
  ...
  leafletCtx.poiLayer = L.layerGroup(
    POIS.map(p => L.marker(toLatLng(p.gx,p.gy))
      .bindPopup(`<strong>${p.name}</strong>${p.note?`<br>${p.note}`:''}`)
      .on("click", () => { selectTile(p.gx,p.gy); renderTile(); }))
  ).addTo(leafletCtx.map);

  // Hide if there's a checkbox and it's off
  const cb = document.getElementById("togglePOI");
  if (cb && !cb.checked){ leafletCtx.map.removeLayer(leafletCtx.poiLayer); leafletCtx.poiLayer = null; }
}
...
modeHistBtn?.addEventListener("click", async ()=>{
  ...
  mapEl.hidden = true;
  lMapEl.hidden = false;
  await initLeafletVisible(); // ensure init happens when visible
});
...
document.getElementById("campBtn")?.addEventListener("click", ()=>{
  ...
  player.stamina += gain; gainXP("fitness", 2); status(`Restored ${gain} stamina.`); renderChar();
});
document.getElementById("townBtn")?.addEventListener("click", ()=>{
  // Go to nearest town (grid)
  let best={x:0,y:0,d:1e9};
  for(let y=0;y<WORLD.height;y++)for(let x=0;x<WORLD.width;x++){
    const t=world.tiles[y][x]; if(t.terrain==="town"){ const d=Math.hypot(player.pos.x-x,player.pos.y-y); if(d<best.d) best={x,y,d}; }
  }
...
document.getElementById("zoom")?.addEventListener("input", (e)=>{
  const z = Number(e.target.value); const size = 14 * z, gap = 2 * z;
...
console.log("Pay Dirt v3.1 — interactive hi-res map fixed (init after visible + invalidateSize).");
