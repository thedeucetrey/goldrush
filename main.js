// Pay Dirt — main.js (full rewrite)
// - Grid gameplay loop unchanged (prospect/pan/sluice, skills, market, claims)
// - Hi-res 1849 map via Leaflet + IIIF (LoC manifest)
// - POIs (Coloma, etc.) clickable → selects nearest grid tile
// - Your claimed tiles render as dots on the 1849 map
...
// Library of Congress IIIF manifest (Wyld, 1849 map)
const HIST_MANIFEST_URL = "https://www.loc.gov/item/99446205/manifest.json";

// Historical POIs roughly mapped to the 64×48 grid
const POIS = [
  { name: "Coloma (Sutter's Mill)", note: "Gold discovery site, Jan 1848", gx: 22, gy: 18 },
  { name: "Placerville (Hangtown)", gx: 24, gy: 19 },
  { name: "Nevada City",            gx: 26, gy: 16 },
  { name: "Downieville",            gx: 30, gy: 12 },
  { name: "Sacramento (Sutter's Fort)", gx: 18, gy: 22 },
  { name: "San Francisco (Yerba Buena)", gx: 5,  gy: 35 },
  { name: "Sonora",                 gx: 36, gy: 24 }
];
...
function genTile(x,y){
  ...
  // Crude river band through the foothills
  ...
  // Gold density (rivers & mountains richer)
  let goldDensity = 0;
  ...
  return {
    x, y,
    terrain: town ? "town" : terrain,
    discovered: false,
    claimedBy: null,
    goldRemaining: town ? 0 : deposit,
...
function buildWorld(){
  world.tiles = Array.from({length: WORLD.height}, (_,y) =>
    Array.from({length: WORLD.width}, (_,x) => genTile(x,y))
  );
}
buildWorld();
...
const initialPlayer = () => ({
  ...
  pos: { x: Math.floor(WORLD.width*0.18), y: Math.floor(WORLD.height*0.68) },
  claims: [],
  store: null
});
...
function load(){
  ...
  if (!s) return false;
  try { const d = JSON.parse(s); world = d.world; player = d.player; status("Loaded local save."); return true; }
  catch { return false; }
}
...
function renderBiz(){
  el("#biz").innerHTML = `
    <p>Create a <strong>General Store</strong> (prototype): Buy low, sell high.</p>
    <div class="row gap">
...
function updateStorePanel(){
  ...
  el("#storePanel").innerHTML = `
    <div class="kv">
      <div><span>Cash on hand</span><span class="stat">$${s.cash?.toFixed?.(2) ?? "0.00"}</span></div>
      <div><span>Inventory (pan)</span><span class="stat">${s.stock?.pan||0}</span></div>
      <div><span>Pan price</span><span class="stat">$${s.prices?.pan?.toFixed?.(2) ?? "45.00"}</span></div>
    </div>
    <div class="row gap" style="margin-top:8px">
...
// ---------- Leaflet IIIF (Hi-Res 1849 Map) ----------
let leaflet = { map:null, iiif:null, playerMarker:null, poiLayer:null, claimsLayer:null };
...
async function initLeaflet(){
  ...
  leaflet = { map, iiif: layer, playerMarker: null, poiLayer: null, claimsLayer: null };

  // Click selects grid tile (proportional mapping)
  map.on("click", (e)=>{
  ...
  // POI toggle (if present in DOM)
  document.getElementById("togglePOI")?.addEventListener("change", (e)=>{
    if (!leaflet.map) return;
    if (e.target.checked) syncPOIsToLeaflet();
    else if (leaflet.poiLayer) { leaflet.map.removeLayer(leaflet.poiLayer); leaflet.poiLayer = null; }
  });
}
...
function syncPOIsToLeaflet(){
  if (!leaflet.map || !leaflet.iiif) return;
  if (leaflet.poiLayer){ leaflet.map.removeLayer(leaflet.poiLayer); leaflet.poiLayer = null; }
  const b = leaflet.iiif._bounds, ne=b.getNorthEast(), sw=b.getSouthWest();
  const toLatLng = (gx,gy) => ([
    sw.lat + (gy + 0.5)/WORLD.height * (ne.lat - sw.lat),
    sw.lng + (gx + 0.5)/WORLD.width  * (ne.lng - sw.lng)
  ]);
  leaflet.poiLayer = L.layerGroup(
    POIS.map(p =>
      L.marker(toLatLng(p.gx,p.gy))
        .bindPopup(`<strong>${p.name}</strong>${p.note?`<br>${p.note}`:""}`)
        .on("click", ()=>{ selectTile(p.gx,p.gy); renderTile(); })
    )
  ).addTo(leaflet.map);

  const cb = document.getElementById("togglePOI");
  if (cb && !cb.checked){ leaflet.map.removeLayer(leaflet.poiLayer); leaflet.poiLayer = null; }
}

function syncClaimsToLeaflet(){
  if (!leaflet.map || !leaflet.iiif) return;
  if (leaflet.claimsLayer){ leaflet.map.removeLayer(leaflet.claimsLayer); leaflet.claimsLayer = null; }
  ...
  leaflet.claimsLayer = L.layerGroup(
    player.claims.map(c => L.circleMarker(toLatLng(c.x,c.y), { radius:4, weight:1 }).bindTooltip("Your claim"))
  ).addTo(leaflet.map);
}
